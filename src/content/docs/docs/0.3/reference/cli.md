...
The server reads `./parqdb-server.toml` when it exists. See [Run a server](/docs/0.3/guides/server) for authorization and deployment guidance.
...
The command never overwrites an existing destination prefix. See [Publish for the browser](/docs/0.3/guides/browser) for the full workflow.
